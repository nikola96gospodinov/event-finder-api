...
async def agent(user_profile: UserProfile, only_highly_relevant: bool = False):
    ...
    # We don't need to pass a keyword for Luma hence the empty string
    event_links = await get_event_links(
        search_keyword="", luma=True, city=user_profile.location.city
    )

    for search_keyword in search_keywords:
        new_links = await get_event_links(
    ...
    for event_link in event_links:
        try:
            event_details = await check_event(event_link, user_profile, gemma_3_27b)
